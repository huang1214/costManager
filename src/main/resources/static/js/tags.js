...
layui.define(['jquery', 'layer'], function(exports) {
    ...
    tags.prototype.addTag = function(obj) {
        ...
        if (text != "") {
            ...
            obj.parent('div.tag-defined').before(tag).empty().css({ "padding": "0 20px" }).html("添加组员学号").prev('.tag').bind('click', { tags: _this }, _this.click).trigger('click');
            old_text = "";
        } else {
            obj.parent('div').empty().css({ "padding": "0 20px" }).html("添加组员学号");
        }
    }
    ...
    tags.prototype.select = function(obj) {
        var val = $.trim(obj.find('.text').text());
